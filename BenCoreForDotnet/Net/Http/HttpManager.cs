...
namespace Ben.Core.Net.Http
{
    ...
    public class HttpManager
    {
        ...
        public static string GetUserAgent()
        {
            ...
            return string.Format("{0}/{1} ({2}; {3})", "ALIAS", "VERSION", "RTFX", osDesc);
        }
        ...
        public static string CreateFormDataBoundary()
        {
            ...
            return string.Format("-------{0}Boundary{1}", "ALIAS", HashUtil.CalcMD5X(now));
        }
        ...
        {
            return requestNonData(NetConst.METHOD_GET, url, token, binaryMode);
        }
        ...
        {
            return requestNonData(NetConst.METHOD_POST, url, token, binaryMode);
        }
        ...
        {
            return PostData(url, data, ContentType.APPLICATION_OCTET_STREAM, token, binaryMode);
        }
        ...
        {
            return postBytesData(url, data, mimeType, token, binaryMode, NetConst.DATA_TYPE_BIN);
        }
        ...
        {
            return postStringData(url, data, ContentType.APPLICATION_JSON, token, binaryMode, NetConst.DATA_TYPE_JSON);
        }
        ...
        {
            return postStringData(url, data, ContentType.TEXT_PLAIN, token, binaryMode, NetConst.DATA_TYPE_TEXT);
        }
        ...
        public HttpResult PostForm(string url, Dictionary<string, string> kvData, string token, bool binaryMode = false)
        {
            ...
            try
            {
                ...
                wReq.Method = NetConst.METHOD_POST;
                if (!string.IsNullOrEmpty(token))
                {
                    wReq.Headers.Add(NetConst.HEADER_AUTHORIZATION, token);
                }
        ...
        {
            return postBytesData(url, data, ContentType.WWW_FORM_URLENC, token, binaryMode, NetConst.DATA_TYPE_FORM);
        }
        ...
        {
            return postBytesData(url, data, string.Format("{0}; boundary={1}", ContentType.MULTIPART_FORM_DATA, boundary), token, binaryMode, NetConst.DATA_TYPE_MULTIPART);
        }
        ...
        /// HTTP方法
        /// </summary>
        /// <param name="method">GET/POST方法</param>
        /// <param name="url">请求目标URL</param>
        /// <param name="token">令牌(凭证)[可选]</param>
        /// <param name="binaryMode">是否以二进制模式读取响应内容(默认:否，即表示以文本方式读取)</param>
        /// <returns>HTTP的响应结果</returns>
        private HttpResult requestNonData(string method, string url, string token, bool binaryMode = false)
        {
            HttpResult result = new HttpResult();
            ...
            try {
                ...
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat("[{0}] [{1}] [HTTP-{2}] Error:  ", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff"), userAgent, method);
                Exception e = ex;
                while (e != null) {
                    sb.Append(e.Message + " ");
                    e = e.InnerException;
        ...
        /// </summary>
        /// <param name="url">请求目标URL</param>
        /// <param name="data">主体数据(字符串数据)</param>
        /// <param name="mimeType">主体数据内容类型</param>
        /// <param name="token">令牌(凭证)[可选]</param>
        /// <param name="binaryMode">是否以二进制模式读取响应内容(默认:否，即表示以文本方式读取)</param>
        /// <returns>HTTP-POST的响应结果</returns>
        private HttpResult postStringData(string url, string data, string contentType, string token, bool binaryMode = false, string dataType = "") {
            HttpResult result = new HttpResult();
            ...
            try {
                ...
                StringBuilder sb = new StringBuilder();
                sb.AppendFormat("[{0}] [{1}] [HTTP-POST-{2}] Error:  ", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff"), userAgent, dataType);
                Exception e = ex;
                while (e != null) {
                    sb.Append(e.Message + " ");
                    e = e.InnerException;
        ...
        /// <param name="binaryMode">是否以二进制模式读取响应内容(默认:否，即表示以文本方式读取)</param>
        /// <returns>HTTP-POST的响应结果</returns>
        private HttpResult postBytesData(string url, byte[] data, string contentType, string token, bool binaryMode = false, string dataType= "") {
            HttpResult result = new HttpResult();
